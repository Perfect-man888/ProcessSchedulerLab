...
- **PCB 全生命周期管理**：创建、编辑、撤销、挂起、激活、搜索和状态筛选。
...
- **批量性能实验**：在隔离副本上对同一数据集运行全部算法，支持实时进度和取消，不影响当前交互仿真。
- **多维指标分析**：统计等待、周转、带权周转、响应、CPU 利用率、吞吐率、切换次数、Deadline Miss 和 Jain 公平性指数。
- **RR 灵敏度扫描**：固定数据集扫描 Quantum 1–8，观察响应时间、周转时间与切换开销的权衡。
- **随机负载生成**：使用泊松到达、截断指数服务时间和固定随机种子生成可复现实验进程。
...
2. 进入“调度仿真”，选择算法并填写该算法需要的参数，然后点击“加载 / 应用算法”。
...
当前共有 **217 项自动化测试**，覆盖数据模型、资源事务、全部调度算法、Priority Aging、上下文切换、I/O 阻塞、随机负载、仿真状态机、后台批量实验、导入导出、PDF 报告、设置持久化及核心 UI 交互；项目要求语句与分支综合覆盖率不低于 90%。
